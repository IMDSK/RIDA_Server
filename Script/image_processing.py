    ...
    for folder_path in folder_paths:
        ...
        for filename in jp2_files:
            if any(res in filename for res, _ in resample_needed):
                ...
                resampled_filename = rename_file(filename, folder_name)
                resample_pairs.append((os.path.join(prepare_dir, filename), os.path.join(rename_dir, resampled_filename), resolution))
        ...
        all_files = [rename_file(f, folder_name) if any(res in f for res, _ in resample_needed) else rename_file(f, folder_name) for f in jp2_files]
